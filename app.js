...
function fillSiteContent() {
  ...
  siteConfig.articles.forEach((item, index) => {
    setText(`[data-article-date="${index}"]`, item.date);
    setText(`[data-article-title="${index}"]`, item.title);
    setText(`[data-article-excerpt="${index}"]`, item.excerpt);
  });

  siteConfig.photos.forEach((photo, index) => {
    setText(`[data-photo-caption="${index}"]`, photo.caption);
  });
...
document.querySelectorAll("[data-article]").forEach((entry) => {
  entry.addEventListener("click", () => {
    openArticle(Number(entry.dataset.article));
  });
});
